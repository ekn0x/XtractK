...
#ifndef TRANSITION_EXIT_ESCAPE_H
#define TRANSITION_EXIT_ESCAPE_H
...
//! \version	1.0
//! 
class TransitionExitEscape : public TransitionOStream
{
public:
	TransitionExitEscape() = delete;
	//! Constructeur requérant le nom, la condition, l'état 
	//! suivant et le flux de sortie. 
	TransitionExitEscape(string const & name, AbstractMatchSymbol * matchSymbol, State * nextState, ostream * stream = &cout);
	//! Destructeur par défaut.
	virtual ~TransitionExitEscape() = default;
...
#endif // TRANSITION_EXIT_ESCAPE_H